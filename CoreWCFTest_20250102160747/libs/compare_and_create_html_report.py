...
def generate_html_report(baseline_data, latest_data, comparison_issues, all_metrics_ok):
    ...
    html_content = [
        ...
        ".success-message { color: green; font-size: 1.2em; font-weight: bold; margin-top: 30px; border: 2px solid green; padding: 10px; background-color: #e6ffe6; }",
        "</style>",
        "</head>",
        "<body>",
    ...
    if all_metrics_ok:
        html_content.append(
            '<div class="success-message">All metrics are within acceptable thresholds. Great job!</div>'
        )
...
def main():
    ...
    try:
        ...
        all_metrics_ok = not comparison_issues  # If no issues, then all metrics are OK
        generate_html_report(baseline_data, latest_data, comparison_issues, all_metrics_ok)
