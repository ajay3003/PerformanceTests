...
def plot_comparison(df):
    ...
    all_metrics_within_threshold = True  # To track if all metrics are within threshold
    ...
    for metric in metrics:
        ...
        # Check if the metric is within the threshold (using hard-coded thresholds)
        if metric == "Average Response Time (ms)" and any(df["Value"] > 10):
            all_metrics_within_threshold = False
        if metric == "Error Rate (%)" and any(df["Value"] > 1):
            all_metrics_within_threshold = False
        if metric == "Throughput (req/sec)" and any(df["Value"] < 5):
            all_metrics_within_threshold = False

    return all_metrics_within_threshold
...
def main():
    ...
    try:
        ...
        if not comparison_data.empty:
            ...
            all_metrics_within_threshold = plot_comparison(comparison_data)
            logging.info(f"Charts have been generated in: {charts_dir}")
            
            # Check and add message if all metrics are within acceptable thresholds
            if all_metrics_within_threshold:
                logging.info("All metrics are within acceptable thresholds.")
                print("\nAll metrics are within acceptable thresholds. Great job!")
            else:
                logging.warning("Some metrics exceeded acceptable thresholds.")
        else:
