...
def compare_metrics(baseline, latest):
    ...
    for label, baseline_metric in baseline_metrics.items():
        if label in latest_metrics:
            ...
            # Check response time
            response_time_diff = abs(latest_metric.get("averageResponseTime", 0) - baseline_metric.get("averageResponseTime", 0))
            if response_time_diff > response_time_threshold:
                issues.append(
                    f"{label}: Response time deviation too high ({latest_metric.get('averageResponseTime', 'N/A')} ms vs {baseline_metric.get('averageResponseTime', 'N/A')} ms)."
                )

            # Check error rate
            error_rate_diff = abs(latest_metric.get("errorRate", 0) - baseline_metric.get("errorRate", 0))
            if error_rate_diff > error_rate_threshold:
                issues.append(
                    f"{label}: Error rate deviation too high ({latest_metric.get('errorRate', 'N/A')} vs {baseline_metric.get('errorRate', 'N/A')})."
                )

            # Check throughput
            throughput_diff = abs(latest_metric.get("throughput", 0) - baseline_metric.get("throughput", 0))
            if throughput_diff > throughput_threshold:
                issues.append(
                    f"{label}: Throughput deviation too high ({latest_metric.get('throughput', 'N/A')} rps vs {baseline_metric.get('throughput', 'N/A')} rps)."
                )
        else:
...
if __name__ == "__main__":
    main()  # Fixed the extra colon here
